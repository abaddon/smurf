...
| `/smurf:nightly-run` denied in auto mode (`autonomous-run.sh` … "autonomous agent loop") | No allow rule for `autonomous-run.sh`; the auto-mode classifier gates the headless `claude -p` run | Run `/smurf:init` in the project — it adds a version-agnostic allow rule to `.claude/settings.local.json` — or approve the command when prompted. |
