...
assert "doctor.sh exits 0 on the development repo" $?

echo
echo "=== init-project.sh ==="
T=$(mktemp -d); CLEANUP_DIRS+=("$T")
bash "$CLAUDE_PLUGIN_ROOT/scripts/init-project.sh" "$T" > /tmp/init-test.out 2>&1
assert "first run exits 0" $?
[ -x "$T/verify.sh" ];                       assert "creates executable verify.sh" $?
[ -f "$T/docs/rigor-level.md" ];             assert "creates docs/rigor-level.md" $?
[ -f "$T/.claude/runs/next-goal.md" ];       assert "creates .claude/runs/next-goal.md" $?
grep -qxF ".claude/runs/" "$T/.gitignore";   assert "gitignore gains .claude/runs/" $?
jq -e '.permissions.allow | length == 2' "$T/.claude/settings.local.json" > /dev/null 2>&1
assert "both allow-rule variants written to settings.local.json" $?

echo "do not overwrite me" > "$T/verify.sh"
bash "$CLAUDE_PLUGIN_ROOT/scripts/init-project.sh" "$T" > /tmp/init-test2.out 2>&1
assert "re-run exits 0 (idempotent)" $?
grep -qx "do not overwrite me" "$T/verify.sh"
assert "re-run does not overwrite existing files" $?
jq -e '.permissions.allow | length == 2' "$T/.claude/settings.local.json" > /dev/null 2>&1
assert "re-run does not duplicate allow rules" $?
[ "$(grep -cxF '.claude/runs/' "$T/.gitignore")" = "1" ]
assert "re-run does not duplicate gitignore lines" $?
...
assert "run with malformed settings.local.json still exits 0" $?
grep -qx '\[\]' "$T2/.claude/settings.local.json"
assert "malformed settings.local.json left untouched" $?
...
[ "$?" = "1" ]; assert "preflight: missing next-goal.md exits 1" $?
...
assert "green run exits 0" $?
[ "$(argval --max-budget-usd "$ARGS")" = "7" ]
assert "budget read from project policy override (7)" $?
[ "$(argval --max-turns "$ARGS")" = "33" ]
assert "max-turns read from project policy override (33)" $?
...
assert "run.ndjson result event parseable" $?
grep -q "| green |" "$PROJ/docs/wiki/log.md" 2>/dev/null
assert "fallback wiki log row appended with status green" $?
git -C "$PROJ" log --oneline | grep -q "docs(wiki): log run"
assert "fallback wiki log row committed" $?
[ -z "$(git -C "$PROJ" status --porcelain -- docs/wiki/log.md 2>/dev/null)" ]
assert "docs/wiki/log.md left clean in the work tree" $?
...
[ "$?" = "124" ]; assert "watchdog fires: exit 124" $?
RUN_DIR2=$(ls -d "$PROJ2"/.claude/runs/2* 2>/dev/null | tail -1)
[ -f "$RUN_DIR2/partial-summary.json" ]
assert "partial-summary.json written on watchdog timeout" $?
grep -q "| terminated |" "$PROJ2/docs/wiki/log.md" 2>/dev/null
assert "fallback wiki log row appended with status terminated" $?
